  ...
  try {
    ...
    if (search) {
      query.$or = [
        ...
        { ntnNumber: { $regex: search, $options: 'i' } }
      ];
    }

    if (type && Object.values(OwnershipType).includes(type as OwnershipType)) {
      query.type = type;
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { hospitalName, type, ntnNumber } = body;

    if (!hospitalName || !type || !ntnNumber) {
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required fields: hospitalName, type, ntnNumber'
        },
        { status: 400 }
      );
    }
    ...
    // Validate enums
    if (!Object.values(OwnershipType).includes(type)) {
      return NextResponse.json(
        {
          success: false,
          error: `Invalid hospital type. Must be one of: ${Object.values(OwnershipType).join(', ')}`
        },
        { status: 400 }
      );
    }
